...
class CustomerController {
    constructor() {
        // 获取房间信息（无需认证）
        this.getRoomInfo = async (req, res) => {
            try {
        ...
        this.getMenu = async (req, res) => {
            try {
                const dishes = await Dish_1.default.find({ isAvailable: true }).select('-__v');
                // 按分类组织菜品
                const categories = {};
                dishes.forEach(dish => {
        ...
        this.createOrder = async (req, res) => {
            try {
                ...
                for (const item of items) {
                    const dish = await Dish_1.default.findById(item.dishId);
                ...
                // 更新房间状态为占用
                await Room_1.default.findByIdAndUpdate(room._id, {
                    status: 'occupied',
        ...
        // 查询订单状态（无需认证，基于房间号）
        this.getOrderStatus = async (req, res) => {
            try {
        ...
        // 获取房间的当前订单
        this.getRoomOrders = async (req, res) => {
            try {
                const { roomNumber } = req.params;
                if (!roomNumber) {
                    res.status(400).json({ error: '房间号不能为空' });
                ...
                // 查找该房间的订单（最近的几个）
                const orders = await Order_1.default.find({ roomNumber })
                    .sort({ createdAt: -1 })
                    .limit(10)
                    .select('-__v');
                res.json({
